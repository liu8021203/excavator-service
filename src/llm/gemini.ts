...
export class GeminiProvider implements LLMProvider {
  ...
  async chat(messages: Message[], options?: LLMOptions): Promise<string> {
    if (!this.ai) {
      throw new Error("Cloudflare Workers AI binding 'AI' is missing in environment (not configured in wrangler.jsonc)");
    }
    ...
    try {
      ...
      throw new Error(`Cloudflare Workers AI Gemini run failed: ${err.message}`);
    }
